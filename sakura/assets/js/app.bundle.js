...
var Libs = function () {
  ...
  function Libs() {
    ...
    this.checkUA = {
      Tablet: this.isUserAgent("windows") != -1 && this.isUserAgent("touch") != -1 || this.isUserAgent("ipad") != -1 || this.isUserAgent("android") != -1 && this.isUserAgent("mobile") == -1 || this.isUserAgent("firefox") != -1 && this.isUserAgent("tablet") != -1 || this.isUserAgent("kindle") != -1 || this.isUserAgent("silk") != -1 || this.isUserAgent("playbook") != -1,
      Mobile: this.isUserAgent("windows") != -1 && this.isUserAgent("phone") != -1 || this.isUserAgent("iphone") != -1 || this.isUserAgent("ipod") != -1 || this.isUserAgent("android") != -1 && this.isUserAgent("mobile") != -1 || this.isUserAgent("firefox") != -1 && this.isUserAgent("mobile") != -1 || this.isUserAgent("blackberry") != -1,
  ...
  _createClass(Libs, [{
    ...
    key: "checkSP",
    value: function checkSP() {
      var sp = this.checkUA.android || this.checkUA.ios || this.checkUA.opera || this.checkUA.windows || this.checkUA.BlackBerry;
      return sp;
    }
    ///////////////////////////
    /// checkIE
...
var _sakura = __webpack_require__(5);

var _sakura2 = _interopRequireDefault(_sakura);
...
var Motion = function () {
  ...
  function Motion() {
    ...
    (0, _jquery2.default)('.btn').on('click', function (e) {
      ...
      (0, _jquery2.default)('.popup').css({
        'display': 'block'
      }).animate({
        'margin-top': -335,
        'opacity': 0.7
      }, 400, 'swing', function (e) {
        (0, _jquery2.default)('.popup').animate({
          'margin-top': -315,
          'opacity': 1
        }, 200, function (e) {

          (0, _jquery2.default)('.popup-front').css({
            'display': 'block'
          });
          (0, _jquery2.default)('#stage').css({
            'display': 'block'
  ...
  _createClass(Motion, [{
    ...
    value: function initCanvas() {
      ...
      for (var i = 0; i < sakuraNum; i++) {
        ...
        var x = Math.floor(stage.width / sakuraNum) * i + objW / 2 + 100;
        var y = Math.floor(Math.random() * (stage.height - 200)) + objW;
        var sakura = new _sakura2.default(x, y);
    ...
    value: function render() {
      ...
      for (var i = 0; i < sakuraArr.length; i++) {
        var sakura = sakuraArr[i];
        sakura.update(i);

        var color = 'rgba(251,231,230,0.8)';
        if (i % 2) {
...
var Sakura = function () {
  ...
  ////////////////////////////
  function Sakura(x, y) {
    _classCallCheck(this, Sakura);

    this.x = x;
    this.y = y;

    this.aDegree = Math.floor(Math.random() * 360);
    var radius = this.radius = Math.floor(Math.random() * 15) + radiusDiff;
    var speedX = this.speedX = (Math.random() * 5 + 1) / 200;
    var effectW = this.effectW = Math.floor(Math.random() * 10) + 10 / 100;
    this.angle = 0;
    var dx = this.dx;
    var pattern = this.pattern = Math.floor(Math.random() * 2);
    this.wind = 0;
  ...
  _createClass(Sakura, [{
    ...
    value: function render(i) {
      ...
      var x = this.x + this.dx + this.wind;
      var y = this.y = this.y + this.speedX * 100;
      ...
      //degree
      //A
      var aDegree = this.aDegree = this.aDegree + this.speedX * 200;
      //B
      this.bDegree = aDegree - 90;
      var bDegree = this.bDegree;
      if (bDegree < 0) bDegree = 360 + bDegree;
      //C
      this.cDegree = bDegree - 70;
      var cDegree = this.cDegree;
      if (cDegree < 0) cDegree = 360 + cDegree;
      //D
      this.dDegree = bDegree - 90;
      var dDegree = this.dDegree;
      if (dDegree < 0) dDegree = 360 + dDegree;
      //E
      this.eDegree = bDegree - 110;
      var eDegree = this.eDegree;
      if (eDegree < 0) eDegree = 360 + eDegree;
      //F
      this.fDegree = dDegree - 90;
      var fDegree = this.fDegree;
      if (fDegree < 0) fDegree = 360 + fDegree;

      //radius
      var radius = this.radius;
      var aPointRadius = this.aPointRadius = radius * 0.8;
      var curvePointRadius = this.curvePointRadius = radius * 0.9;
      var notchPointRadius = this.notchPointRadius = radius - radius * 0.3;

      //radian
      //A
      this.aRadian = aDegree * (Math.PI / 180);
      var aRadian = this.aRadian;
      //B
      this.bRadian = bDegree * (Math.PI / 180);
      var bRadian = this.bRadian;
      //C
      this.cRadian = cDegree * (Math.PI / 180);
      var cRadian = this.cRadian;
      //D
      this.dRadian = dDegree * (Math.PI / 180);
      var dRadian = this.dRadian;
      //E
      this.eRadian = eDegree * (Math.PI / 180);
      var eRadian = this.eRadian;
      //F
      this.fRadian = fDegree * (Math.PI / 180);
      var fRadian = this.fRadian;
